...
type EntryView = 'splash' | 'welcome' | 'signin' | 'signup' | 'account-type' | 'plan' | 'payment' | 'family-choice' | 'create-family' | 'join-family' | 'families'
...
export default function Onboarding({ onComplete, initialView = 'splash' }: { onComplete: (family: FamilyConnection) => void; initialView?: EntryView }) {
  ...
  if (view === 'plan') {
    ...
    return <main className="fc-onboarding-shell"><section className="fc-onboarding-card fc-plan-card"><button className="fc-back-link" type="button" onClick={() => go('account-type')}>← Back</button><p className="fc-kicker">Account setup · Step 3</p><h1>Choose your Family Circle plan</h1><p className="fc-onboarding-copy">{accountType === 'child' ? 'Choose the plan for your child’s account. Child accounts include family safety controls.' : 'Choose the plan for this account. You can change your plan later.'}</p><div className="fc-plan-grid">{plans.map((plan) => <button key={plan.id} className={selectedPlan === plan.id ? 'fc-plan-option selected' : 'fc-plan-option'} type="button" onClick={() => setSelectedPlan(plan.id)} aria-pressed={selectedPlan === plan.id}><div className="fc-plan-option-head"><span><strong>{plan.name}</strong><small>{plan.price}</small></span><b>{selectedPlan === plan.id ? '✓' : '→'}</b></div><ul>{plan.features.map((feature) => <li key={feature}>{feature}</li>)}</ul></button>)}</div>{error && <p className="fc-entry-error">{error}</p>}<button className="fc-primary-button fc-plan-continue" type="button" disabled={!selectedPlan} onClick={() => selectedPlan === 'free' ? go('family-choice') : go('payment')}>Continue</button><p className="fc-plan-note">{selectedPlan === 'free' ? 'Free has no payment step.' : selectedPlan ? 'You’ll review payment details next.' : 'Select a plan to continue.'}</p></section></main>
  }

  if (view === 'payment') {
    const planName = selectedPlan === 'premium' ? 'Premium' : 'Plus'
    const planPrice = selectedPlan === 'premium' ? '£4.99/month' : '£2.49/month'
    return <main className="fc-onboarding-shell"><section className="fc-onboarding-card fc-payment-card"><button className="fc-back-link" type="button" onClick={() => go('plan')}>← Back</button><p className="fc-kicker">Account setup · Payment</p><h1>Complete your subscription</h1><p className="fc-onboarding-copy">You’ve selected {planName} at {planPrice}. Payment will be securely connected when billing is added.</p><div className="fc-payment-summary"><div><span>Selected plan</span><strong>{planName}</strong></div><div><span>Price</span><strong>{planPrice}</strong></div></div><div className="fc-payment-placeholder"><span aria-hidden="true">💳</span><strong>Secure payment setup</strong><p>Card payment will be added in the billing stage. No payment is taken on this screen yet.</p></div><button className="fc-primary-button fc-payment-continue" type="button" onClick={() => go(accountType === 'child' ? 'family-choice' : 'family-choice')}>Continue</button></section></main>
  }
