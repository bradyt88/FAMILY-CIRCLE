...
export default function Onboarding({ onComplete, initialView = 'splash' }: { onComplete: (family: FamilyConnection) => void; initialView?: EntryView }) {
  ...
  if (view === 'payment') {
    ...
    return <main className="fc-onboarding-shell"><section className="fc-onboarding-card fc-payment-card"><button className="fc-back-link" type="button" onClick={() => go('plan')}>← Back</button><p className="fc-kicker">Account setup · Payment</p><h1>Complete your subscription</h1><p className="fc-onboarding-copy">You’ve selected {planName} at {planPrice}. This is a demo checkout. Real card billing will be securely connected when payments are added.</p><div className="fc-payment-summary"><div><span>Selected plan</span><strong>{planName}</strong></div><div><span>Price</span><strong>{planPrice}</strong></div></div><div className="fc-payment-placeholder"><span aria-hidden="true">💳</span><strong>Demo payment</strong><p>Enter your card details here in the future. This demo does not take or store a real payment.</p></div><button className="fc-primary-button fc-payment-continue" type="button" onClick={() => go(accountType === 'child' ? 'child-access' : 'family-choice')}>Start subscription — {planPrice}</button></section></main>
  }
