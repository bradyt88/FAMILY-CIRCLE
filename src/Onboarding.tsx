...
export default function Onboarding({ onComplete, initialView = 'splash' }: { onComplete: (family: FamilyConnection) => void; initialView?: EntryView }) {
  ...
  if (view === 'payment') {
    ...
    return <main className="fc-onboarding-shell"><section className="fc-onboarding-card fc-payment-card"><button className="fc-back-link" type="button" onClick={() => go('plan')}>← Back</button><p className="fc-kicker">Account setup · Payment</p><h1>Complete your subscription</h1><p className="fc-onboarding-copy">You’ve selected {planName} at {planPrice}. Payment will be securely connected when billing is added.</p><div className="fc-payment-summary"><div><span>Selected plan</span><strong>{planName}</strong></div><div><span>Price</span><strong>{planPrice}</strong></div></div><div className="fc-payment-placeholder"><span aria-hidden="true">💳</span><strong>Secure payment setup</strong><p>Card payment will be added in the billing stage. No payment is taken on this screen yet.</p></div><button className="fc-primary-button fc-payment-continue" type="button" onClick={() => go(accountType === 'child' ? 'child-access' : 'family-choice')>Continue</button></section></main>
  }
